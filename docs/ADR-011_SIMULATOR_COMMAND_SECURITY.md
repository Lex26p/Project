...
S37 exposes no execute method or executor. S38 may execute only after repeating lease/session/permission/config/current/safety preflight; timeout remains unknown until reconciliation. Protocol assemblies remain read-only, physical qualification remains absent until explicit `IG-08` authorization, and production physical writes remain disabled through `S43` unless separately approved.

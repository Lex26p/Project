...
**Статус программы:** `S07` реализован и проверен на Windows x64; остановлено перед `S08`

**Последний завершённый пакет:** `S07` — Facility/Equipment identity and ownership foundation (`ee503f9` + working tree, commit не создан)

## Следующая работа

`S08` — permission-filtered Location/Equipment Server→Web projections из `./DISPATCHER_SPRINT_CATALOG.md`. В текущей работе не начат.

Windows x64 evidence для `S07`: locked restore и format verification green; Release build — 0 warnings/0 errors; 39 unit + 16 integration tests green. Integration tests использовали отдельный временный PostgreSQL cluster без Docker.
...
- module persistence schemas beyond implemented Platform/Personal Workspace/Facility/Equipment owners;
